...
public class SubwayBuild {
	...
	// private String station_r = null;// 역의 정보를 담고있는 파일의 경로를 저장.
	// private String subway_r = null;// (지하철)경로의 정보를 담고있는 파일의 경로를 저장.
	private String temp_s;// 문자열을 읽을 변수.
	private int staCnt, subCnt;//역과 경로의 수량을 저장하는 변수.
	// 생성자 메서드.
	public SubwayBuild(/* String station_r, String subway_r */) throws IOException {
		// this.station_r = station_r;
		// this.subway_r = subway_r;
		// 임시변수 초기화
		staCnt = 0;//역
		subCnt = 0;//경로
		temp_s = null;
	...
	public void StationB() throws IOException {
		// 데이터 읽어 오기.
		BufferedReader stationData = new BufferedReader(// 역의 데이터 파일 읽기.
				new FileReader("C:\\Users\\Sol\\eclipse-workspace\\Shortest Path\\StationData.txt"));
		...
		// 데이터 다시 읽어 오기.
		stationData = new BufferedReader(// 역의 데이터 파일 다시 읽기.
				new FileReader("C:\\Users\\Sol\\eclipse-workspace\\Shortest Path\\StationData.txt"));
		// 데이터 입력 단계
		while ((temp_s = stationData.readLine()) != null) {
	...
	public void SubwayB() throws IOException {
		// 데이터 읽어 오기.
		BufferedReader subwayData = new BufferedReader(// 경로의 데이러 파일 읽기.
				new FileReader("C:\\Users\\Sol\\eclipse-workspace\\Shortest Path\\SubwayData.txt"));
		...
		// 데이터 다시 읽어 오기.
		subwayData = new BufferedReader(// 경로의 데이러 파일 읽기.
				new FileReader("C:\\Users\\Sol\\eclipse-workspace\\Shortest Path\\SubwayData.txt"));
		// 데이터 입력 단계.
		while ((temp_s = subwayData.readLine()) != null) {
			String[] split = temp_s.split("	");// tap으로 구분.
